...
# PROPER METADATA from seed_db_production.py
games_raw = [
    {
...
async def run_import():
    ...
    try:
        ...
        db.execute(text("TRUNCATE TABLE game_stats, signups, games RESTART IDENTITY CASCADE"))
        db.commit()

        # Ensure archive chat exists
        archive_chat_id = -1000000000001
        chat_res = db.execute(select(Chat).where(Chat.chat_id == archive_chat_id))
        ...
        # Build user map
        users_res = db.execute(select(User))
        ...
        for g_data in games_raw:
            ...
            cnt = 0
            cnt += add_players(w_list, Team.A)
            cnt += add_players(l_list, Team.B)
            cnt += add_players(d_list, Team.A) # Draws on A
