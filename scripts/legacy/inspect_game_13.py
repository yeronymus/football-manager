import asyncio
import os
import sys
from sqlalchemy import select, text
from sqlalchemy.ext.asyncio import create_async_engine, AsyncSession
from sqlalchemy.orm import sessionmaker
from app.db.models import Game, Signup, User

# Setup minimal DB connection
DATABASE_URL = os.getenv("DATABASE_URL")
if not DATABASE_URL:
    # Fallback to config default if env not set in shell
    DATABASE_URL = "postgresql+asyncpg://postgres:password@db:5432/football"

engine = create_async_engine(DATABASE_URL)
async_session = sessionmaker(engine, expire_on_commit=False, class_=AsyncSession)

async def inspect():
    async with async_session() as session:
        print("--- INSPECTING GAME 13 ---")
        game = await session.get(Game, 13)
        if not game:
            print("Game 13 NOT FOUND")
            return
        
        print(f"Game: {game.id}")
        print(f"Date: {game.date_time}")
        print(f"Location: {game.location}")
        print(f"Slots: {game.max_players}")
        print(f"Format Info: {game.chat_id} | {game.message_id} | {game.admin_message_id}")
        
        # Get Signups
        stmt = select(Signup, User).join(User).where(Signup.game_id == 13).order_by(Signup.created_at)
        result = await session.execute(stmt)
        rows = result.all()
        
        print(f"\nTotal Signups Found: {len(rows)}")
        for i, (signup, user) in enumerate(rows, 1):
            print(f"{i}. {user.full_name} (@{user.username}) - Status: {signup.status} - Paid: {signup.is_paid}")

if __name__ == "__main__":
    import asyncio
    asyncio.run(inspect())
