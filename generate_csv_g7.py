#!/usr/bin/env python3
"""Generate FM_Player_Stats.csv with Game 7 logic."""
import csv
...
POS_GRP = {
    "GK":"0ВРТ","CB":"1ЗАЩ","LB":"1ЗАЩ","RB":"1ЗАЩ",
    "CM":"2МИД","CDM":"2МИД","CAM":"2МИД","LM":"2МИД","RM":"2МИД",
    "FWD":"3АТК","LW":"3АТК","RW":"3АТК","ST":"3АТК"
}
...
for g in games_raw:
    ...
    for p in w_team:
        if p not in stats: stats[p] = {"elo": 100, "gp":0, "g":0, "w":0, "l":0, "d":0, "mvp":0, "logs": [""]*7}
        stats[p]["gp"] += 1; stats[p]["w"] += 1; stats[p]["elo"] += 10
        pgc = goals.get(p, 0)
        stats[p]["g"] += pgc
        stats[p]["logs"][g["id"]-1] = f"{pgc}g W"
    for p in l_team:
        if p not in stats: stats[p] = {"elo": 100, "gp":0, "g":0, "w":0, "l":0, "d":0, "mvp":0, "logs": [""]*7}
        ...
        if g["id"] == 1 and p in ["Arsen Kudaibergen","Aidyn Galymbekov","Umid Kamilov","Daryn Bakyrkhan","Zhanibek Xenbek","Aktan Abdullaatov"]:
            ...
            pgc = goals.get(p, 0)
            stats[p]["g"] += pgc
            stats[p]["logs"][g["id"]-1] = f"{pgc}g D"
        else:
            stats[p]["elo"] -= 5
            pgc = goals.get(p, 0)
            stats[p]["g"] += pgc
            stats[p]["logs"][g["id"]-1] = f"{pgc}g L"
...
all_players = sorted(stats.keys(), key=lambda x: (
    POS_GRP.get(PLAYER_PROPS.get(x, {"pos":"CM"})["pos"] if x.lower() != "nurken samigullin" else "RB", "2МИД"),
    -stats[x]["elo"],
    -stats[x]["g"]
))

with open("/home/yeronym/Documents/fmBot/football-manager/FM_Player_Stats.csv", "w", encoding="utf-8") as f:
    writer = csv.writer(f)
    writer.writerow(["Группа", "Игрок", "Поз", "Альт. Поз", "ELO", "Games", "Goals", "W", "L", "D", "Win%", "MVP", "Game 1", "Game 2", "Game 3", "Game 4", "Game 5", "Game 6", "Game 7"])
    for p in all_players:
        st = stats[p]
        props = PLAYER_PROPS.get(p, {"pos": "CM", "alt":""})
        if p.lower() == "nurken samigullin": props = {"pos": "RB", "alt": "LB, CM"}
        
        grp_full = POS_GRP.get(props["pos"], "2МИД")
        grp = grp_full[1:] # remove sorting digit
        winpct = calc_win_pct(st['w'], st['d'], st['l'])
        writer.writerow([grp, p, props["pos"], props["alt"], st["elo"], st["gp"], st["g"], st["w"], st["l"], st["d"], winpct, st["mvp"]] + st["logs"])

print("Created FM_Player_Stats.csv")
