...
async def run_restore_history():
    async with async_session_maker() as session:
        print("=== 🛠 RESTORING HISTORY RECORDS ONLY (SAFE) 🛠 ===")
        ...
        for g_data in ALL_GAMES:
            ...
            print(f"Restoring Game ID {g_data['id']} ({dt})...")
            ...
            for team_enum, t_info in teams_map.items():
                for p_name in t_info["members"]:
                    ...
                    # 4. Add RatingHistory record (for /my_history visibility)
                    change = 0
                    ...
                    session.add(RatingHistory(
                        ...
                        old_rating=0, new_rating=0, change=change
                    ))
            
        await session.commit()
        print("=== ✨ HISTORY RESTORED! NO MMR CHANGED ✨ ===")

if __name__ == "__main__":
    asyncio.run(run_restore_history())
