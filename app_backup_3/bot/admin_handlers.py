...
async def cmd_add_player(message: types.Message, session: AsyncSession):
    ...
    try:
        ...
        async with UnitOfWork() as uow:
            # Re-fetch user in this Session/UOW if needed, but RosterService might work with detached object 
            # if we just need ID and attributes. 
            # Safest is to merge or re-fetch if simple join.
            # RosterService.join_player takes `user` object.
            # Let's trust it works or re-fetch.
            # Actually, `target_user` is from `session`.
